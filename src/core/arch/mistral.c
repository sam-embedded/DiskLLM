...
static bool mistral_init(diskllm_model *model) {
    (void)model;
    return true;
}

static bool mistral_prefill_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_states, int prompt_len) {
...
static bool mistral_decode_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_single, int cur_pos) {
    diskllm_model *model = ctx->model;
    const qwen_model_config *cfg = model->cfg;
    const layer_block_weights_internal *blk = (const layer_block_weights_internal *)layer_weights;
    scratch_buffers *scratch = ctx->scratch;
...
const diskllm_arch_backend diskllm_arch_mistral = {
    .name = "Mistral",
    .init = mistral_init,
    .prefill_layer = mistral_prefill_layer,
    .decode_layer = mistral_decode_layer
};
