...
static bool llama_init(diskllm_model *model) {
    (void)model;
    return true;
}

static bool llama_prefill_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_states, int prompt_len) {
...
static bool llama_decode_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_single, int cur_pos) {
    diskllm_model *model = ctx->model;
    const qwen_model_config *cfg = model->cfg;
    const layer_block_weights_internal *blk = (const layer_block_weights_internal *)layer_weights;
    scratch_buffers *scratch = ctx->scratch;
...
const diskllm_arch_backend diskllm_arch_llama = {
    .name = "Llama",
    .init = llama_init,
    .prefill_layer = llama_prefill_layer,
    .decode_layer = llama_decode_layer
};
