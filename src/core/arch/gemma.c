...
static bool gemma_prefill_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_states, int prompt_len) {
    ...
    for (int pos = 0; pos < prompt_len; pos++) {
        ...
        rmsnorm_ext(scratch->hidden_state, h, blk->post_attn_norm_w, cfg->hidden_dim, 1e-6f, 1);
...
static bool gemma_decode_layer(diskllm_context *ctx, int layer_idx, const void *layer_weights, float *hidden_single, int cur_pos) {
    ...
    rmsnorm_ext(scratch->hidden_state, hidden_single, blk->post_attn_norm_w, cfg->hidden_dim, 1e-6f, 1);
    matvec(scratch->ffn_gate, blk->ffn_gate_w, scratch->hidden_state,
           cfg->hidden_dim, cfg->ffn_dim, blk->ffn_gate_w_type, scratch->ssm_qkv);
    matvec(scratch->ffn_up,   blk->ffn_up_w,   scratch->hidden_state,
           cfg->hidden_dim, cfg->ffn_dim, blk->ffn_up_w_type,   scratch->ssm_qkv);
